...
CImg<T> promedio(CImgList<T> lista_imagenes) {
  ...
  for ( unsigned i=1; i<tam_lista; i++ ) {
    ...
    promediada += lista_imagenes(i) / (double)tam_lista;
  }

  // christian habias hecho:  
  // for (int i = 1; i < tam_lista; i++) { 
  //   cimg_forXY(promediada,x,y) 
  //     {
  // 	promediada(x, y) += lista_imagenes[i](x, y);
  // 	promediada(x,y)/=2.0; // estaba mal esto, porque estás ponderando igual el promedio que ya acumulaste
  // 	                         igual que la imagen por la que vas pasando, i.e. las primeras imagenes casi
  // 	                         ni cuentan en el promedio final.
  //     }
  // }
  return promediada;
}
...
double calcular_mse( CImgList<T> imagenes, CImgList<T> patrones) {
  ...
  for ( unsigned i=0; i<patrones.size(); i++ ) {
    // valido que las imágenes a comparar tengan las mismas dimensiones ( x, y, c )
    if ( imagenes(i).width() != patrones(i).width() ||
	 imagenes(i).height() != patrones(i).height() ||
	 imagenes(i).spectrum() != patrones(i).spectrum() )
      return -1.0;

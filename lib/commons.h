 ...
 * (imagen) que es la que luego se usar� para comparar contra una imagen dada.
 ...
 * de -abstracci�n- de los procesos aplicados a la misma.
 * TODO: hacer parametros configurables
 */
template<class T>
CImg<T> generar_base(CImgList<T> lista) {
	return segmentar(detectar_bordes(promedio(lista), 2.0, 70.0). normalize(0,
			255), 100, 100).get_append('x');
}

/**
 * generar_base
 * genera una imagen base con todas las im�genes que encuentre en el directorio
 * pasado como par�metro.
 * @param directorio directorio donde buscar las im�genes para generar la base
...
CImg<T> generar_base(const char* directorio) {
	...
	// ac� lo que hago es expandir la expresi�n (directorio)/*, y para cada una
	// de las expansiones matcheo la expresi�n regular "match", que matchea para
	// los archivos tipo imagen: .jpg, .jpeg, .JPG, .bmp, et cetera.

	// globbuf es la estructura donde se guardan los resultados del glob.
	// me interesan 2 de sus miembros: gl_pathc, que contiene el n�mero
	// de expansiones, y gl_pathv que es un char** con las expansiones mismas
	glob_t globbuf;

	// match contendr� la expresi�n regular "compilada" en la llamada a regcomp
	regex_t match;

	// compilo la expresi�n regular en el 2do par�metro en match,
	// el 3er argumento le dice que ignore mayusculas/minusculas
	// por alguna put�sima raz�n tengo que comentar con \\ los caracteres ()?|
 ...
 * @param directorio directorio donde buscar las im�genes
 * @return lista de im�genes cargada del directorio 
 */
template<class T>
...
/**
 * sumar_dim
 * agarra una imagen y la suma a lo largo de una dimensi�n, i.e. si sumo
 * a lo largo de x entonces obtendr� un vector en y del mismo alto que la imagen
 * original, donde cada punto en y contendr� el valor de sum(0:ancho,y)
 * @param imagen la imagen sobre la que calcular la suma
 * @param dimension la dimensi�n a sumar, por defecto x, valores posibles: 'xXyY'
 * @return vector de la suma, con una dimensi�n menos que la imagen original.
...
/**
 * estadisticas_imagen
 * calcula una serie de estad�sticas sobre la imagen pasada como par�metro, a saber
 ...
 * sumas horiz y vert.
 * @param imagen la imagen sobre la que se calcular�n estas medidas
...
CImg<double> estadisticas_imagen(const CImg<T> imagen) {
	...
	// relleno el vector con resultados. uso sqrt(var) = stddev porque es m�s
 ...
 * calcula el MSE entre los vectores de estad�sticas de dos im�genes.
 * @param img1, img2 las im�genes a comparar

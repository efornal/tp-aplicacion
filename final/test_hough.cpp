...
int main(int argc, char **argv) {
  ...
  for ( int i=0; i< base_dir.size(); i++ ) {
    ...
    comp.ponderacion(0) = 0.0; // anulamos estadisticas
    comp.ponderacion(1) = 2.0; // ponderamos por 2 por la division
    ...
    t = comp.clasificar_directorio( string( prueba_dir[i] ).c_str() , nombres, clases );

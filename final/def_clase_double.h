#ifndef DEF_CLASE_DOUBLE_H
#define DEF_CLASE_DOUBLE_H
...
struct ExtractorCaracts {
  string nombre;
  virtual CImg<double> extraer_caracteristicas( CImg<double> imagen );
};
...
class ComparadorImagenes {
 ...
 public:
  ...
  int encontrar_mas_parecida ( CImg<double> imagen );
...
// que encuentra en directorio.
//template<class T>
vector<string> ComparadorImagenes::listar_imagenes( const char* directorio ) {
...
// devuelve el mse entre los dos conjs de caracteristicas
//template<class T>
double ComparadorImagenes::comparar_caracteristicas ( vector<CImg<double> > img,
					    unsigned idx ) {
  double resultado = 0;
...
//template<class T>
ComparadorImagenes::ComparadorImagenes() { n_caracteristicas=0; n_imagenes=0; }

//template<class T>
ComparadorImagenes::~ComparadorImagenes() { }

  // agrega el extractor de caracteristicas pasado de parametro
  // para comparacion
//template<class T>
int ComparadorImagenes::agregar_extractor( ExtractorCaracteristicas extractor,
					      double ponderacion ) {
...
//       (no será que puedo hacer de las 2 una y chau?)
//template<class T>
int ComparadorImagenes::cargar_imagenes( const char* directorio ) {
  int i=0;
...
// la base.
//template<class T>
int ComparadorImagenes::calcular_caracteristicas ( ) {
  int cont=0, k;
  CImg<double> imagen_temp;
...
// dada una imagen, encuentra el indice de la que es mas parecida
//template<class T>
int ComparadorImagenes::encontrar_mas_parecida ( CImg<double> imagen ) {

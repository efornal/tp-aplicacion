...
parámetros. En este trabajo, utilizamos el espacio de los parámetros de rectas.}
que son principales en la imagen.

Para la extraccion de características con esta técnica se siguen los siguientes
pasos:
\begin{enumerate}
...
\item Se obtiene una imagen de sólo bordes aplicando el operador gradiente de
      Sobel (\ref{sob}) y luego se umbraliza según \ref{umbral}.
\item Con la imagen umbralizada se calcula la transformada de Hough.
...
Así se obtiene un vector de 45 caracterśticas a partir de histogramas que se

...
Como se puede observar en la \resalt{figura x}, en el primer paso se obtiene una imagen de un solo canal mediante el promediado de los tres canales RGB. Luego, con el objetivo de disminuir el costo computacional, se realiza un redimensionamiento de la imagen de 640 x 480 pixels a 100x100. Posteriormente para la detección de bordes se aplica el operador gradiente de sobel definido por:
$$
\nabla f \approx \mid G_x \mid + \mid G_y \mid
$$
$$
G_x = \left[
\begin{array}{ccc}
...
\right]
$$
Con el objetivo de lograr una imagen binaria, se umbraliza asignandole el valor 0 a los pixels cuyo valor de intensidad es menor a \resalt{cuanto}, mientras que a los demás se le asigna el valor 1. Tras estos pasos se transforma al espacio de Hough y aquí se aplica nuevamente un redimensionado a tamaño de 40x40 con el fin de \resalt{no se como poner acá lo de la tol. de los grados}
...
\resalt{habría que ponerlo alado al gy}
\resalt{agregar gráfica proceso hough}
\begin{figure}
...
Las imágenes sobre las cuál se aplicaron los algoritmos, fueron tomadas en la ciudad de Santa Fe, mediante un Dispositivo Móvil con una resolución de 640x480 pixels. Se construyo una base de datos de imágenes sobre seis edificios, tomando trece realizaciones de cada uno de ellos (diez con el propósito de usarlas como prototipo y tres para la prueba con los algoritmos).
